...
const TabMilestones: React.FC<{
  ...
  const [isManageDeliverablesVisible, setIsManageDeliverablesVisible] =
    useState(false);
  const [
    selectedMilestoneForDeliverables,
    setSelectedMilestoneForDeliverables,
  ] = useState<any>(null);
  ...
  const handleShowCountdownChange = (e: any) => {
    ...
    localStorage.setItem(
      KEY_SHOW_MILESTONE_COUNTDOWN,
      JSON.stringify(newValue)
    );
  };

  const handleManageDeliverables = (
  ...
  return (
    <>
      <div className="mb-4 px-4">
        <Checkbox checked={showCountdown} onChange={handleShowCountdownChange}>
          Show Countdown
        </Checkbox>
      </div>
      <Timeline className="mt-4 px-4">
        {project.milestones
          ...
          .map((milestone, index) => {
            const isExpanded = expandedMilestones.includes(
                        ...
                        <>
                          ...
                          {milestone.status != MilestoneStatusDto.Pending && (
                            <div className="mt-4">
                              ...
                              milestone.deliverables.filter((d) => d.isVisible)
                                .length > 0 ? (
                                <FileList
                                  files={milestone.deliverables.filter(
                                    (d) => d.isVisible
                                  )}
                                />
                              ) : (
